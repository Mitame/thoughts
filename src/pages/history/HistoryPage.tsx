...
import { Thought, ThoughtStorageContext } from "../../providers/ThoughtStorageProvider"
...
export default function HistoryPage() {
    ...
    useEffect(() => {
        thoughtStorage.retrieveThoughts().then((thoughts) => setThoughts(thoughts)).catch((e) => {
            console.error(e);
        })
    }, [thoughtStorage])
    ...
    return <div className={styles.page}>
        {thoughtsGroupedByDate.map(({thoughts, date}) => <ThoughtGroup key={date} thoughts={thoughts} />)}
    </div>
}
...
export function ThoughtGroup({thoughts}: {thoughts: Thought[]}) {
    ...
    return <>
        ...
        {thoughts.map((thought, i) => {
            ...
            const isNewSession = !previousThought || (thought.timestamp.valueOf() - previousThought.timestamp.valueOf()) > 5 * 60 * 1000;

            const time = timeFormatter.format(thought.timestamp);

            return <Fragment key={i}>
                <span className={styles.time} data-new-session={isNewSession}>{time}</span>
